...
exports.createSummary = function (req, res, next) {
  ...
  models.User.findOne({ where: { profileid: email } })
    .then(user => {
      if (!user) {
        return res.status(404).json({ error: 'User not found' })
      }

      // Update the req's authenticated user
      req.user = user

      // Proceed with the note creation using the updated req.user
      return noteUtil.newNote(req, res, body, email)
    })
    .catch(err => {
